...
struct capability_info vm_exit[17] =
{
	{ 2, "Save debug controls" },
	{ 9, "Host address-spac size" },
...
struct capability_info vm_entry[13] =
{
...
struct capability_info procbased3[4] =
{
	{ 0, "LOADIWKEY exiting " },
	{ 1, "Enable HLAT " },
...
detect_vmx_features(void)
{
	...
	pr_info("\nVM-Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_exit, 17, lo, hi);
	
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("\nVM-Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_entry, 13, lo, hi);
	...
	pr_info("\nTertiary Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbased3, 4, lo, hi);*/
}
